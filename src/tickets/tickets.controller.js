import { AppError, catchAsync } from "../errors/index.js";
import { validatePartialTicket, validateTicket } from "./tickets.schema.js";
import { TicketService } from "./tickets.services.js";

const ticketService = new TicketService();

export const findAllTickets = catchAsync(async (req, res, next) => {
  const tickets = await ticketService.findAll();

  return res.status(200).json(tickets);
});

export const createTicket = catchAsync(async (req, res, next) => {
  const { errorMessage, hasError, ticketData } = validateTicket(req.body);
  const { id: sessionUserId } = req.sessionUser;

  if (hasError) {
    return res.status(422).json({
      status: "error",
      message: errorMessage,
    });
  }

  const allData = {
    createdBy: sessionUserId,
    ...ticketData,
  };

  const ticket = await ticketService.create(allData);
  return res.status(201).json(ticket);
});

export const findOneTicket = catchAsync(async (req, res, next) => {
  const { id } = req.params;

  const ticket = await ticketService.findOne(id);

  if (!ticket) {
    return next(new AppError("Ticket not found", 404));
  }

  return res.status(200).json(ticket);
});

export const updateTicket = catchAsync(async (req, res, next) => {
  const { errorMessage, hasError, ticketData } = validatePartialTicket(
    req.body
  );

  if (hasError) {
    return res.status(422).json({
      status: "error",
      message: errorMessage,
    });
  }

  const { id } = req.params;

  const ticket = await ticketService.findOne(id);

  if (!ticket) {
    return next(new AppError("Ticket not found", 404));
  }

  const ticketUpdate = await ticketService.update(ticket, ticketData);

  return res.status(200).json(ticketUpdate);
});

export const deleteTicket = catchAsync(async (req, res, next) => {
  const { id } = req.params;

  const ticket = await ticketService.findOne(id);

  if (!ticket) {
    return next(new AppError("Ticket not found", 404));
  }

  await ticketService.delete(ticket);

  return res.status(204).json(null);
});
